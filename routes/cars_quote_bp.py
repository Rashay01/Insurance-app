...
def get_cars_quote():
    ...
    data = [
        cars_quote.to_dict() for cars_quote in cars_quote_list
    ] 
    return jsonify(data)
...
def update_specific_cars_quote(vehicleId,quoteId):
    ...
    try:
        ...
        db.session.commit()
        result = {"message": "Cars Quote link updated successfully", "data": cars_quote.to_dict()}
...
def delete_specific_cars_quote(vehicleId,quoteId):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message":"Cars Quote link successfully deleted","data":data})
    except Exception as e:
        db.session.rollback()
        return {"message": "Error occurred","Error": str(e)}, 500
...
def add_cars_quote():
    ...
    try:
        ...
        db.session.commit()
        result = {"message": "Cars Quote link added successfully", "data": new_cars_quote.to_dict()}
